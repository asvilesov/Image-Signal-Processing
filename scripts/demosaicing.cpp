...
int main(int argc, char *argv[]){
    ...
    for(auto i = 0; i < img1.height; i++){
        for(auto j = 0; j<img1.width; j++){
            ptr = bilin_img.ptr(i, j);
			   ptr[0] = img1.rgb_pixels[i][j][0];
            ptr[1] = img1.rgb_pixels[i][j][1];
            ptr[2] = img1.rgb_pixels[i][j][2];
        }
    }

    //MHC Demosaicing
    image img2(input,height,width);
    demosaicingMHC(img2);
    ptr = nullptr;
    ...
    for(auto i = 0; i < img2.height; i++){
        for(auto j = 0; j<img2.width; j++){
            ptr = mhc_img.ptr(i, j);
			ptr[0] = img2.rgb_pixels[i][j][0];
            ptr[1] = img2.rgb_pixels[i][j][1];
            ptr[2] = img2.rgb_pixels[i][j][2];
        }
    }
